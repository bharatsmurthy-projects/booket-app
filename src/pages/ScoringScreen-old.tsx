...
import type { MatchState, ImpactCardEffect, AnimationType, ActiveEffect, BallEvent, OverSummary, InningsData } from '../types';
import {
  ...
  processActiveEffectsBeforeBall, processActiveEffectsAfterBall,
  decrementActiveEffects, applyDoubleWicketEffect,
} from '../lib/gameEngine';
...
import ImpactCardScreen   from '../components/ImpactCardScreen';
    ...
    <div className="scoring-screen">
      ...
      <div className="scoring-panel">
        ...
        {mode === 'impact' && (
          <ImpactCardScreen 
            innings={innings}
            onApply={(effect, activeEffect) => {
              if (effect === 'active_effect' && activeEffect) {
                // POWER CARDS / FREE HIT
                const inningsKey = match.currentInnings === 1 ? 'innings1' : 'innings2';
                const currentInnings = match[inningsKey];
                const over = currentInnings.currentOver!;
                
                // Create new effect with ID
                const newEffect: ActiveEffect = {
                  id: `${activeEffect.type}_${Date.now()}`,
                  type: activeEffect.type,
                  ballsRemaining: activeEffect.ballsRemaining,
                  description: activeEffect.description,
                  ...(activeEffect.value && { value: activeEffect.value }),
                  ...(activeEffect.penalty && { penalty: activeEffect.penalty }),
                };
                ...
                // Update over with impact ball
                const updatedOver: OverSummary = {
                  ...over,
                  balls: [...over.balls, impactBall],
                  impactApplied: true,
                };
                
                // Update innings with new effect AND the impact ball
                const updatedInnings: InningsData = {
                  ...currentInnings,
                  currentOver: updatedOver,
                  activeEffects: [...(currentInnings.activeEffects || []), newEffect],
                };
                ...
                // REGULAR IMPACTS (+6, -3, OUT, ×2)
                handleImpactCard(effect as ImpactCardEffect);
              }
            }}
          />
        )}
